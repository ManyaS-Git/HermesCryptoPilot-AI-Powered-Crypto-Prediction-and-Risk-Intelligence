...
class BacktestingService:
    ...
    async def run_replay(self, asset: str, timeframe: str, limit: int = 1000):
        ...
        """
        logger.info(
            f"Starting backtest replay for {asset} on {timeframe} (Limit: {limit})"
        )

        # 1. Fetch historical data
        try:
            pair = f"{asset}USDT" if "USDT" not in asset else asset
            data: List[OHLCV] = await self.market_data.fetch_ohlcv(
        ...
        if len(data) < 512:
            logger.warning(
                f"Not enough data for a robust backtest. Got {len(data)} bars."
            )
        ...
        logger.info(
            f"Backtest complete. Simulated Accuracy: {accuracy:.2f}, Brier Score: {brier_score:.2f}"
        )
        return {"asset": asset, "accuracy": accuracy, "brier_score": brier_score}
