...
class KronosService:
    ...
    def _load_model(self):
        logger.info(
            f"Loading Kronos model: {self.model_name} and tokenizer: {self.tokenizer_name}"
        )
    ...
    async def predict(self, ohlcv_data: List[OHLCV]) -> KronosPrediction:
        ...
        _window = self._window_builder(features)  # used for real model later
        ...
        logger.info(
            f"Kronos prediction result: {prediction.predicted_move} ({prediction.raw_probability})"
        )
        return prediction
