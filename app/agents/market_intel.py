...
class MarketIntelAgent:
    ...
    async def get_market_consensus(self, asset: str) -> UnifiedMarketConsensus:
        ...
        if not valid_probs:
            logger.warning(f"No API odds found for {asset}. Falling back to Apify sentiment scraping.")
        ...
        logger.info(f"Market consensus for {asset}: {unified_prob:.4f} (Sources: {sources})")
